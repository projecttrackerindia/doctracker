...
function buildExportPdfContentHtml(proj, endpoints, opts){
  ...
  const orgLabel = pdfOrgLabel();
  const logoDataUrl = pdfLogoDataUrl();
    ...
    <section class="pdf-cover">
      <div class="pdf-letterhead">
        <div class="pdf-letterhead-brand">
          ${logoDataUrl ? `<img class="pdf-letterhead-logo" src="${logoDataUrl}" alt="">` : `<div class="pdf-letterhead-logo pdf-letterhead-logo-fallback">${escapeHtml((orgLabel || '?').charAt(0).toUpperCase())}</div>`}
          <div>
            <div class="pdf-letterhead-org-name">${escapeHtml(orgLabel)}</div>
            <div class="pdf-letterhead-org-sub">API Documentation Export</div>
          </div>
        </div>
        <div class="pdf-letterhead-meta">
          <div><span class="k">Generated by</span>${escapeHtml(author)}</div>
