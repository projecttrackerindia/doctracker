...
function buildExportPdfContentHtml(proj, endpoints, opts){
  ...
  return `
    ${coverHtml}
    ${endpointsHtml}
    <div class="pdf-atom pdf-atom-tight"><div class="pdf-footer">${escapeHtml(proj.name)} · Generated by DocTracker · ${escapeHtml(generatedAtStr)} · by ${escapeHtml(state.authorName || 'Unknown')}</div></div>
  `;
}
