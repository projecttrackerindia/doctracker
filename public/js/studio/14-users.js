...
function renderEndpointDoc(main, proj, ep){
  ...
  function responsePanelContent(i){
    ...
    if(activeTab === 'json'){
      ...
      return `
        <div class="resp-status-heading"><span class="code-pill ${respClass(code)}">${escapeHtml(String(code))}</span><span class="txt">${matchedScenario ? escapeHtml(matchedScenario.description || matchedScenario.name) : (r.description ? escapeHtml(r.description) : 'Response JSON')}</span></div>
