...
function swaggerSample(proj, ep){
  ...
  // Kept in sync with the same normalized-name matching in server/openapiExport.js —
  // an exact-string check on just "authorization"/"x-api-key"/"api-key" missed any
  // other spelling (Auth-Token, ClientID, Bearer, JWT, ...), so a header documented
  // on the endpoint page never made it into the Swagger security requirement here.
  ...
  const normalizeHeaderName = n => String(n||'').trim().toLowerCase().replace(/[^a-z0-9]/g,'');
  const isAuthHeaderName = n => authHeaderNames.has(normalizeHeaderName(n));
  const rawHeaders = ep.headers || [];
  const authHeader = rawHeaders.find(h => isAuthHeaderName(h.name));
  // NOTE: isAuthHeaderName() below is only for deciding the `security`
  // requirement (see securityScheme below). It must NOT also strip the
  // header out of what gets rendered as a parameter — a header like
  // CLIENT-ID or CLIENT-SECRET is both "evidence this endpoint needs auth"
  // AND a real header the caller has to send; OpenAPI's http/apiKey security
  // schemes can't represent it on their own. Filtering it out of
  // normalHeaders previously made it vanish from `parameters` entirely
  // (Swagger then showed "No parameters" even though it was configured on
  // the endpoint page). Render every header as a parameter; auth-shaped
  ...
  const headerParamRefs = normalHeaders.map(h=>{
    ...
    const maskThis = h.example && (isSensitiveHeaderName(h.name) || isAuthHeaderName(h.name)) && !swaggerRevealed;
