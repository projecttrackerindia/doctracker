...
/* ---------- Request flow direction: custom preset builder wiring ----------
   Lives in the Add/Edit endpoint modal's "Project details" section now
   (Project settings no longer edits this) — falls back to whatever project
   is currently backing that modal (see currentEndpointProject, set by
   hydrateEndpointProjectFields/clearEndpointProjectFields). */
document.getElementById('mFlowDirection').addEventListener('change', (e)=>{
  const builder = document.getElementById('mFlowCustomBuilder');
  if(e.target.value === '__add_custom__'){
    builder.style.display = 'block';
    document.getElementById('mFlowCustomLabel').focus();
  } else {
    builder.style.display = 'none';
  }
});
document.getElementById('mFlowCustomCancel').addEventListener('click', ()=>{
  document.getElementById('mFlowCustomBuilder').style.display = 'none';
  populateFlowDirectionSelect(currentEndpointProject ? (currentEndpointProject.requestFlowDirection || '1-way') : '1-way', 'mFlowDirection');
});
document.getElementById('mFlowCustomAdd').addEventListener('click', ()=>{
  const label = document.getElementById('mFlowCustomLabel').value.trim();
  if(!label){ toast('Enter a label for the custom direction'); return; }
  const pattern = document.getElementById('mFlowCustomPattern').value === '2-way' ? '2-way' : '1-way';
  ...
  document.getElementById('mFlowCustomBuilder').style.display = 'none';
  document.getElementById('mFlowCustomLabel').value = '';
  populateFlowDirectionSelect(id, 'mFlowDirection');
