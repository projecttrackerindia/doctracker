...
// brand-new one). Used so the "Request flow direction" custom-preset builder
// can fall back to this project's saved direction on Cancel, the same way it
// used to read editingProjectId in Project settings.
...
function hydrateEndpointProjectFields(proj){
  ...
  populateFlowDirectionSelect(proj.requestFlowDirection || '1-way', 'mFlowDirection');
...
function clearEndpointProjectFields(){
  ...
  populateFlowDirectionSelect('1-way', 'mFlowDirection');
  document.getElementById('mFlowCustomBuilder').style.display = 'none';
...
function saveManualEndpoint(){
  ...
  const mFlowVal = document.getElementById('mFlowDirection').value;
  proj.requestFlowDirection = (mFlowVal && mFlowVal !== '__add_custom__') ? mFlowVal : (proj.requestFlowDirection || '1-way');
...
function populateFlowDirectionSelect(selectedId, selectId){
  const sel = document.getElementById(selectId || 'mFlowDirection');
  if(!sel) return;
