...
function renderDocAccessTab(body){
  ...
  let hasMore = false;
  let cursorId = null;
  let cursorPending = null; // whether the last-shown row was in the pending group — /admin sorts pending-first, so the cursor needs this too, not just the id (see docAccess.js)
  ...
  function renderTableAndFooter(){
    ...
    if(loadMoreBtn) loadMoreBtn.addEventListener('click', async ()=>{
      ...
      try{
        const page = await apiGet(`/doc-access/admin?limit=50&cursorId=${cursorId}&cursorPending=${cursorPending}`);
        all = all.concat(page.requests);
        hasMore = page.hasMore;
        if(page.requests.length){
          const last = page.requests[page.requests.length - 1];
          cursorId = last.id;
          cursorPending = last.status === 'pending';
        }
  ...
  Promise.all([
    ...
    cursorPending = all.length ? all[all.length - 1].status === 'pending' : null;
