...
// reencryptOrganisation() (server/routes/workspace.js) if you want a given
// organisation's rows done immediately instead of lazily — note that's
// per-organisation, not a whole-database reencryptAll().
//
...
// changing it needs a config change + restart. What the Admin UI exposes as
// "Rotate encryption key" is DEK rotation (rotateDataKey(), below), which is
// instant and needs no deploy — that's the one that matters for "our key may
// be compromised."
//
// HYGIENE (finding 6b): this comment used to say "see rotateMasterKey()
// below" as if that procedure already existed in this file — it never did.
// Only rotateDataKey() (DEK rotation, exposed via the Admin UI's "Rotate
// encryption key" — instant, no deploy needed) and reencryptOrganisation()
// exist today. Master-key rotation itself is NOT currently implemented
// anywhere in this codebase; it would need to be done manually (decrypt
// every DEK with the old MASTER_KEY, re-wrap with the new one, in a script
// run outside the request/response cycle) if it's ever needed. Until that
// script exists, an engineer responding to a suspected MASTER_KEY
// compromise should treat this as a genuine gap, not a documented
// procedure they just haven't found yet.
// ============================================================================
...
// ---- Buffer-native envelope encryption (no base64 double-encoding) ----
// encryptField is string-oriented: it base64s the plaintext into a Buffer,
// AES-encrypts that, then base64s the iv/ciphertext/tag again to join them
// with ':'. For raw binary payloads (file attachments) that's two stacked
// base64 hops — roughly 1.77x the original byte count — for no benefit,
// since the caller already has real bytes and doesn't need a ':'-delimited
// string.
//
// Binary token layout (all fixed-width, no delimiters, no base64):
//   [2 bytes: key version, big-endian] [12 bytes: IV] [16 bytes: GCM tag] [ciphertext]
// AES-GCM is a stream cipher (no block padding), so ciphertext is exactly
// plaintext.length bytes — total overhead is a flat 30 bytes per blob,
// versus ~1.77x under encryptField.
function encryptBuffer(plaintextBuf, aad) {
  const dek = getDek(activeVersion);
  const { iv, ct, tag } = aesEncrypt(dek, plaintextBuf, aad);
  const header = Buffer.alloc(2);
  header.writeUInt16BE(activeVersion, 0);
  return Buffer.concat([header, iv, tag, ct]);
}

// Distinguishes a binary token (above) from an old encryptField() text token
// during migration. Text tokens are UTF-8 starting with the ASCII bytes
// "enc:v" — first byte 0x65 ('e'). A binary token's first byte is the high
// byte of a uint16 key version, which is 0x00 for every version below 256
// (i.e. in practice, always) — the two formats can never collide.
function isBinaryEncrypted(buf) {
  return Buffer.isBuffer(buf) && buf.length > 2 + IV_LEN + 16 && buf[0] !== 0x65;
}

function decryptBuffer(token, aad) {
  const version = token.readUInt16BE(0);
  const iv = token.subarray(2, 2 + IV_LEN);
  const tag = token.subarray(2 + IV_LEN, 2 + IV_LEN + 16);
  const ct = token.subarray(2 + IV_LEN + 16);
  const dek = getDek(version);
  return aesDecrypt(dek, { iv, ct, tag }, aad);
}
