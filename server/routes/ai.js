...
router.get('/config', async (req, res) => {
  try {
    const { rows } = await pool.query(
      `SELECT provider, model, api_key_enc, api_key_last4, updated_at FROM org_ai_settings WHERE organisation = $1`,
      [req.authUser.organisation]
    );
    const row = rows[0];
    const resolvedProviderId = row?.provider || 'anthropic';
    res.json({
      // Bug fix: this used to check `row.api_key_enc !== null` without
      // ever selecting api_key_enc in the query above, so row.api_key_enc
      // was always `undefined` — and `undefined !== null` is `true` in JS,
      // so this was `true` for ANY org with a settings row at all, even
      // one where the key had been removed (DELETE /config nulls both
      // columns, but this stayed true regardless). That's very likely what
      // was actually behind "I can't see my AI key" reports: the org's
      // "configured" banner kept claiming a key was on file long after it
      // was gone, so the real state only became visible again once someone
      // re-entered a key. api_key_last4 alone is both correct and
      // sufficient — it's null exactly when there's no key, full stop.
      configured: !!(row && row.api_key_last4),
