...
// Was: /^(authorization|x-api-key|api-key)$/i — an EXACT, case-insensitive
// match on only those three literal names. A header added on an endpoint
// under any other spelling (Auth-Token, X-Auth-Token, ClientID,
// Client-Secret, Bearer, JWT, Access-Token, ...) never matched, so it stayed
// a plain documented header instead of becoming a security requirement —
// which is why "auth added on the endpoint" showed up on the doc page
// (which lists every header regardless of name) but never in the Swagger
// render (which only ever looked for those three names). Normalizing away
// case/spaces/punctuation and matching against a wider set of known
// auth-header spellings fixes this without requiring an exact name.
function normalizeHeaderName(name) {
  return String(name || '').trim().toLowerCase().replace(/[^a-z0-9]/g, '');
}
const AUTH_HEADER_NAMES = new Set([
  'authorization', 'apikey', 'xapikey', 'authtoken', 'xauthtoken',
  'bearertoken', 'accesstoken', 'xaccesstoken', 'clientid', 'clientsecret',
  'jwt', 'xjwt', 'jwttoken',
]);
function isAuthHeaderName(name) {
  return AUTH_HEADER_NAMES.has(normalizeHeaderName(name));
}
...
function buildOperation(ep, schemas) {
  ...
  const authHeader = rawHeaders.find((h) => isAuthHeaderName(h.name));

  // NOTE: isAuthHeaderName() is used below (and by the caller) purely to
  // decide whether this operation should get `security: [{ApiAuth: []}]`
  // attached. It must NOT also be used to drop the header from the
  // rendered `parameters` list — that was the bug: a header like CLIENT-ID
  // or CLIENT-SECRET is both "evidence this endpoint needs auth" AND a real
  // header the caller has to send, and OpenAPI's http/apiKey security
  // schemes can't represent arbitrary extra credential headers like these
  // on their own. Dropping them left `parameters: []` with no trace of
  // CLIENT-ID/CLIENT-SECRET anywhere in the generated spec, even though
  // they were configured on the endpoint (see the Headers section in the
  // DocTracker UI). So: render every header as a parameter; auth-shaped
  // ones are just also masked, same as any other sensitive header.
  const normalHeaders = rawHeaders;
...
function buildProjectOpenApiSpecYaml(proj, envList) {
  ...
  if (anyAuthHeader) {
    ...
    for (const ep of endpoints) {
      if (!ep || !ep.path) continue;
      const rawHeaders = ep.headers || [];
      const hasAuth = rawHeaders.some((h) => isAuthHeaderName(h.name));
      if (hasAuth) {
        const methodLower = (ep.method || 'get').toLowerCase();
  ...
  const banner = `# ${proj.name || 'Untitled API'} — full project spec\n`
    + `# Generated by DocTracker — shared link, hosts and secrets are always masked.\n\n`;
  try {
