/* ==================== SECTION:AUTH ====================
   Injected server-side from the signed session — see server/server.js.
   Real identity (username/organisation/role) comes from here, not localStorage. */
...
function toggleSensitiveRevealed(){
  ...
  requestSensitiveReveal('Endpoint doc — sensitive values', ()=>{
...
// ---------- Request flow direction: built-in presets + user-defined custom ones ----------
// Every preset resolves to an underlying arrow "pattern" (1-way / 2-way) that the request-flow
// SVG knows how to draw; custom presets just give that pattern their own label, and are saved
// to localStorage so they're available across every project.
const FLOW_DIRECTION_PRESETS = [
  { id:'1-way', label:'One-way — request only (Client → Gateway → Flow → Downstream)', pattern:'1-way' },
  { id:'2-way', label:'Two-way — request and response (Client ⇄ Gateway ⇄ Flow ⇄ Downstream)', pattern:'2-way' },
];
const CUSTOM_FLOW_DIRECTIONS_KEY = 'apiStudio.customFlowDirections'; // only used by the one-time localStorage migration now
function saveCustomFlowDirections(list){
  state.customFlowDirections = list;
  apiSend('PUT', '/custom-flow-directions', { customFlowDirections: list })
    .catch(e=>{ console.error('Save flow direction presets failed', e); toast('Could not save flow direction presets to the server.'); });
}
function allFlowDirections(){ return FLOW_DIRECTION_PRESETS.concat(state.customFlowDirections || []); }
function resolveFlowDirection(proj){
  const id = (proj && proj.requestFlowDirection) || '1-way';
  return allFlowDirections().find(d=>d.id===id) || FLOW_DIRECTION_PRESETS[0];
}
...
function roleAllowedEnvs(roleId){
  ...
  return role.seesAll ? environments() : environments().filter(e=>!e.restricted);
}
...
/* Lightweight markdown renderer for description/notes text pulled in from OpenAPI specs
   (headings, numbered/bulleted lists, bold, italics, inline code, paragraphs). Input is
   escaped first so this never introduces raw HTML — only the whitelisted tags below are emitted. */
...
async function loadState(){
  ...
  // not the old client-writable workspace blob.
  try{
...
function setAuthorRole(roleId){
  ...
  if(roleId !== 'admin'){ state.sensitiveRevealed = false; state.envUrlRevealed = {}; }
...
// Everything gated purely by role — hide the create FAB and disable the environment
// options a Viewer/Developer shouldn't reach. This is a UI convenience, not a security
// boundary (there's no backend here to enforce it against).
...
function parseSpecToProject(spec){
  ...
  return {
    ...
    lifecycle: 'DEVELOPMENT', owner:'', team:'', requestFlowDirection: '1-way', createdAt: now, updatedAt: now,
...
function exportBackup(){
  ...
  a.href = url;
  a.download = `api-studio-backup-${new Date().toISOString().slice(0,10)}.json`;
  a.click();
...
function headerList(proj, ep){
  ...
  const headers = [];
  if(ep.requestBody) headers.push(['Content-Type', ep.contentType || 'application/json']);
  if(proj.auth && proj.auth.headerName){
    headers.push([proj.auth.headerName, authHeaderValue(proj)]);
  }
      ...
      <div class="env-dd-panel" id="envDDPanel" role="listbox" aria-label="Environment" tabindex="-1">
        ${environments().map(e=>{
          ...
          const hoverHint = locked
            ? `Your role (${roleMeta(state.authorRole).label}) doesn't have access to ${e.label}`
...
function selectEnvironment(envId){
  ...
  if(!roleAllowsEnv(envId)){
    toast(`Your role (${roleMeta(state.authorRole).label}) doesn't have access to ${envMeta(envId).label}`);
    return;
  }
  state.env = envId;
  saveEnv();
...
function renderSidebar(){
  ...
  list.innerHTML = projects.map(proj=>{
    ...
    const groupsToShow = (filter && !hasEpMatches && projMatches) ? groupByTag(proj.endpoints) : groups;
    ...
    const tagsHtml = Object.keys(groupsToShow).sort().map(tag=>{
      const eps = groupsToShow[tag].slice().sort((a,b)=>a.path.localeCompare(b.path));
    ...
    return `<div class="project ${isOpen?'open':''}" data-proj="${proj.id}">
      <div class="project-head" data-toggle="${proj.id}" style="--proj-accent:${accent};--proj-accent-bg:${accentBg};">
        ...
        <div class="project-actions">
          <span class="project-count">${proj.endpoints.length}</span>
...
function renderMain(){
  ...
  const found = findEndpoint(state.selected.id);
  if(!found){ state.selected = null; renderMain(); return; }
...
function renderControlCenter(main){
  ...
  const envRows = environments().map(e=>{
    let configured = 0;
    projects.forEach(p=>{
      if(p.environments[e.id]) configured++;
    });
    const pct = projects.length ? Math.round((configured/projects.length)*100) : 0;
    return `<tr>
      <td><span class="env-chip" style="--env-accent:${envAccentColor(e.id)};--env-accent-bg:${envBgColor(e.id)};">${e.label}</span></td>
      <td class="mono">${projects.length}</td>
      <td class="mono">${pct}%</td>
      <td class="mono" style="color:${pct===100?'var(--post)':pct===0?'var(--text-faint)':'var(--put)'};">${configured}/${projects.length || 0} APIs</td>
    </tr>`;
  }).join('');
  ...
  const projectRows = projects.length ? projects.map(p=>{
    ...
    const envsConfigured = environments().filter(e=>p.environments[e.id]).length;
    return `<tr class="cc-proj-row" data-cc-proj="${p.id}">
      ...
      <td class="mono">${envsConfigured}/${environments().length}</td>
  ...
  main.innerHTML = `
    ...
    <div class="cc-hero">
      ...
      <div class="cc-hero-sub">A live rollup computed from everything documented in this workspace — no external monitoring involved.</div>
    </div>
    ...
    <div class="kpi-grid">
      ...
      ${kpiCard('Fully configured APIs', m.envsFullyConfigured, 'var(--get)', 'all '+environments().length+' environments set')}
    ...
    <div class="section">
      ...
      <table class="data-table">
        <thead><tr><th>Environment</th><th>APIs</th><th>% configured</th><th>Coverage</th></tr></thead>
        <tbody>${envRows}</tbody>
      </table>
    </div>
    ...
    <div class="section">
      ...
      <table class="data-table cc-proj-table">
        <thead><tr><th>Name</th><th>Lifecycle</th><th>Endpoints</th><th>Documentation</th><th>Environments</th><th>Owner</th></tr></thead>
...
function userRowHtml(u){
  ...
  <tr class="env-row">
    ...
    <td><span class="role-pill role-${rMeta.id}"${customTitle}>${rMeta.label}</span></td>
...
let userModalState = { mode:'invite', user:null, role:'viewer', envs:[], canEditFlag:false };
...
async function saveUserModal(){
  ...
  if(userModalState.mode === 'invite'){
    ...
    body = { username, email, role, customPermissions };
  ...
  try{
    ...
    if(userModalState.mode === 'invite'){
      usersState.list.push(data.user);
      ...
      if(idx !== -1) usersState.list[idx] = data.user;
...
function renderProjectOverview(main, projectId){
  ...
  const totalEndpoints = proj.endpoints.length;
  const envsConfigured = environments().filter(e=>proj.environments[e.id]).length;
  const tagCount = new Set(proj.endpoints.map(e=>e.tag || 'General')).size;
  ...
  main.innerHTML = `
    ...
    <div class="overview-hero">
      <div class="overview-hero-badge"><span class="dot"></span>API Studio · ${escapeHtml(env.label)} environment</div>
    ...
    <div class="section">
      <div class="section-title">Request flow <span style="color:var(--text-faint); font-weight:500; text-transform:none;">— ${isCustomFlowPreset ? escapeHtml(flowPreset.label) : (direction==='2-way' ? 'two-way (request &amp; response)' : 'one-way (request only)')}, set from Edit settings</span></div>
    ...
    <div class="section">
      ...
      </table>` : `<div class="empty-field">No endpoints in this project yet.</div>`}
    </div>
    ...
    <div class="section">
      ...
      ${proj.auth && proj.auth.type ? `
        ...
        ${proj.auth.includeInDocs ? paramSection('Auth request parameters', proj.auth.requestParams||[]) : ''}
        ${proj.auth.includeInDocs ? paramSection('Auth response parameters', proj.auth.responseParams||[]) : ''}` : `<div class="auth-card"><div class="ic">🔓</div><div><div class="h">No auth configured</div><div class="d">Add authentication details in project settings.</div></div></div>`}
    </div>
...
function paramSection(title, params, variant){
  // variant: 'header' -> blue tint for Header parameters; a status code (e.g. 200/404) ->
...
function renderEndpointDoc(main, proj, ep){
  ...
  const hasAnyParams = pathParams.length || queryParams.length;
  const paramsHtml = hasAnyParams ? [
    paramSection('Path parameters', pathParams),
    paramSection('Request parameters', queryParams),
  ].join('') : `<div class="empty-field">No parameters documented.</div>`;

  const headersHtml = headerParams.length ? paramSection('Header parameters', headerParams, 'header') : '';
  const hasReqJson = !!(ep.requestBody && ep.requestBody.example);
  const reqExampleList = [
    { label:'Default', value: (ep.requestBody && ep.requestBody.example) || '' },
    ...((ep.requestBody && ep.requestBody.examples) || []).map(ex=>({ label: ex.name || 'Example', value: ex.value })),
  ];

  const requestAccordionHtml = `
    <div class="resp-item open" data-request-item>
      <div class="resp-row" data-request-toggle>
        <span class="resp-code" style="background:var(--accent-soft);color:var(--accent);">REQ</span>
        <span class="resp-desc">Path, query &amp; body parameters for this request</span>
        <span class="resp-caret">▶</span>
      </div>
      <div class="resp-body">
        <div class="resp-tabs">
          <button type="button" class="resp-tab active" data-request-tab="params">Request parameters</button>
          <button type="button" class="resp-tab" data-request-tab="json"${hasReqJson ? '' : ' disabled title="No request JSON documented"'}>Request JSON${reqExampleList.length>1 ? ` (${reqExampleList.length} examples)` : ''}</button>
        </div>
        <div class="resp-tab-panel" data-request-panel>${paramsHtml}</div>
      </div>
    </div>`;

  const responsesHtml = ep.responses && ep.responses.length
    ? ep.responses.map((r,i)=>{
        const respExampleCount = 1 + ((r.examples||[]).length);
        return `
      <div class="resp-item" data-resp="${i}">
        <div class="resp-row" data-resp-toggle="${i}">
          <span class="resp-code ${respClass(r.code)}">${escapeHtml(String(r.code))}</span>
          <span class="resp-desc">${r.description ? escapeHtml(r.description) : '<span class="empty-field">No description</span>'}</span>
          <span class="resp-caret">▶</span>
        </div>
        <div class="resp-body">
          <div class="resp-tabs">
            <button type="button" class="resp-tab active" data-resp-tab="params" data-resp-idx="${i}">Response parameters</button>
            <button type="button" class="resp-tab" data-resp-tab="json" data-resp-idx="${i}">Response JSON${respExampleCount>1 ? ` (${respExampleCount} examples)` : ''}</button>
          </div>
          <div class="resp-tab-panel" data-resp-panel="${i}">
            ${r.fields && r.fields.length ? paramSection('Response parameters', r.fields, r.code) : `<div class="empty-field">No response parameters documented.</div>`}
          </div>
        </div>
      </div>`;}).join('')
        ...
        <div class="path-actions">
          ...
          <div class="ep-actions-dd" id="epActionsDD">
            ...
            <div class="ep-actions-panel" id="epActionsPanel" role="menu">
              ...
              <div class="ep-actions-divider"></div>
              <button type="button" class="ep-actions-item" id="btnEditEp" role="menuitem"${canEdit()?'':' disabled title="Your role is read-only"'}>Edit</button>
              <button type="button" class="ep-actions-item" id="btnDuplicateEp" role="menuitem"${canEdit()?'':' disabled title="Your role is read-only"'}>Duplicate</button>
              ${proj._readonly ? '' : `
              <div class="ep-actions-divider"></div>
  ...
  document.getElementById('btnTryIt').addEventListener('click', ()=>openTryItModal(proj, ep));
  ...
  document.getElementById('btnEditEp').addEventListener('click', ()=>{
    document.getElementById('epActionsDD').classList.remove('open');
    if(canEdit()) openManualModal(ep.id);
  });
  document.getElementById('btnDuplicateEp').addEventListener('click', ()=>{
    document.getElementById('epActionsDD').classList.remove('open');
    if(canEdit()) duplicateEndpoint(ep.id);
  });
  ...
  main.querySelectorAll('[data-request-tab]').forEach(btn=>{
    btn.addEventListener('click', (e)=>{
      e.stopPropagation();
      if(btn.disabled) return;
      const tab = btn.getAttribute('data-request-tab');
      const panel = main.querySelector('[data-request-panel]');
      btn.parentElement.querySelectorAll('.resp-tab').forEach(b=>b.classList.remove('active'));
      btn.classList.add('active');
      if(tab === 'json'){
        if(!hasReqJson){
          panel.innerHTML = `<div class="empty-field">No request JSON documented.</div>`;
          return;
        }
        panel.innerHTML = `
          <div class="json-preview-label">
            <span>Request JSON${reqExampleList.length>1 ? ' — pick an example' : ''}</span>
            ${reqExampleList.length>1 ? `<select class="render-example-select" id="docReqExampleSelect">${reqExampleList.map((ex,i)=>`<option value="${i}">${escapeHtml(ex.label)}</option>`).join('')}</select>` : ''}
          </div>
          <div class="code-wrap"><pre class="code-block" id="docReqJsonBlock">${escapeHtml(maskedJsonString(reqExampleList[0].value))}</pre><button class="copy-btn" data-copy-block="reqbody">Copy</button></div>`;
        let current = maskedJsonString(reqExampleList[0].value);
        const cp = panel.querySelector('[data-copy-block="reqbody"]');
        if(cp) cp.addEventListener('click', (ev)=>copyToClipboard(current, ev.currentTarget));
        const sel = panel.querySelector('#docReqExampleSelect');
        if(sel) sel.addEventListener('change', ()=>{
          current = maskedJsonString(reqExampleList[parseInt(sel.value,10)].value);
          panel.querySelector('#docReqJsonBlock').textContent = current;
        });
      } else {
        panel.innerHTML = paramsHtml;
      }
    });
  });
  ...
  main.querySelectorAll('[data-resp-toggle]').forEach(el=>{
    el.addEventListener('click', ()=>{
      el.closest('.resp-item').classList.toggle('open');
    });
  });
  main.querySelectorAll('[data-resp-tab]').forEach(btn=>{
    btn.addEventListener('click', (e)=>{
      e.stopPropagation();
      const idx = parseInt(btn.getAttribute('data-resp-idx'),10);
      const tab = btn.getAttribute('data-resp-tab');
      const r = ep.responses[idx];
      const panel = main.querySelector(`[data-resp-panel="${idx}"]`);
      btn.parentElement.querySelectorAll('.resp-tab').forEach(b=>b.classList.remove('active'));
      btn.classList.add('active');
      if(tab === 'json'){
        if(!r.example){
          panel.innerHTML = `<div class="empty-field">No example body.</div>`;
          return;
        }
        const respExampleList = [{ label:'Default', value: r.example }, ...((r.examples)||[]).map(ex=>({ label: ex.name || 'Example', value: ex.value }))];
        panel.innerHTML = `
          <div class="json-preview-label">
            <span>Response JSON${respExampleList.length>1 ? ' — pick an example' : ''}</span>
            ${respExampleList.length>1 ? `<select class="render-example-select" id="docRespExampleSelect-${idx}">${respExampleList.map((ex,i)=>`<option value="${i}">${escapeHtml(ex.label)}</option>`).join('')}</select>` : ''}
          </div>
          <div class="code-wrap"><pre class="code-block" id="docRespJsonBlock-${idx}">${escapeHtml(maskedJsonString(respExampleList[0].value))}</pre><button class="copy-btn" data-copy-resp="${idx}">Copy</button></div>`;
        let current = maskedJsonString(respExampleList[0].value);
        const cp = panel.querySelector(`[data-copy-resp="${idx}"]`);
        if(cp) cp.addEventListener('click', (ev)=>copyToClipboard(current, ev.currentTarget));
        const sel = panel.querySelector(`#docRespExampleSelect-${idx}`);
        if(sel) sel.addEventListener('change', ()=>{
          current = maskedJsonString(respExampleList[parseInt(sel.value,10)].value);
          panel.querySelector(`#docRespJsonBlock-${idx}`).textContent = current;
        });
      } else {
        panel.innerHTML = r.fields && r.fields.length
          ? paramSection('Response parameters', r.fields, r.code)
...
async function requestSensitiveReveal(context, onGranted, onAutoRemask){
  ...
  logSecurityEvent('PII_REVEAL', { entityType:'security', entityName: context, severity:'warning', metadata:{ field: context, reason } });
  onGranted();
...
function newExampleRow(){ return { id:uid(), name:'', value:'' }; }
  ...
  containerEl.innerHTML = examples.map((ex,i)=>`
    <div class="example-card" data-ex-id="${ex.id}">
      <div class="example-card-head">
        ...
        <input type="text" data-exf="name" value="${escapeHtml(ex.name)}" placeholder="e.g. Subscription renewal">
...
function openManualModal(epId){
  ...
  if(editingEndpointId){
    ...
    builderReqExamples = (ep.requestBody && ep.requestBody.examples || []).map(ex=>({id:uid(), name:ex.name||'', value:ex.value||''}));
...
function saveManualEndpoint(){
  ...
  // The API-level description lives on the project, not the endpoint — persist
  // it here so it always lands in info.description of every generated YAML,
  // no separate trip to Project settings required.
...
async function deleteEndpointFromModal(){
  ...
  if(state.selected && state.selected.type==='endpoint' && state.selected.id===editingEndpointId){
...
function updateEnvSettingsSummary(){
  const total = environments().length;
  const configured = document.querySelectorAll('#envInputs [data-env-input]').length
    ? Array.from(document.querySelectorAll('#envInputs [data-env-input]')).filter(i=>i.value.trim()).length
    : 0;
  const summary = document.getElementById('envSettingsSummary');
  if(!summary) return;
  const pct = total ? Math.round((configured/total)*100) : 0;
  summary.innerHTML = `<span class="n">${configured}/${total}</span> environments configured
    <span class="bar"><span class="bar-fill" style="width:${pct}%;"></span></span>`;
}

function renderEnvSettingsCards(proj){
  const envInputs = document.getElementById('envInputs');
  envInputs.innerHTML = environments().map(e=>{
    const val = proj.environments[e.id] || '';
    const has = !!val;
    return `
    <div class="env-settings-card" style="--esc-accent:${envAccentColor(e.id)};">
      <div class="esc-head">
        <div class="esc-label-wrap">
          <span class="esc-dot"></span>
          <span class="esc-label">${e.label}</span>
        </div>
        <span class="esc-status ${has ? 'set' : 'unset'}" data-esc-status>${has ? 'Configured' : 'Not set'}</span>
      </div>
      <div class="esc-body">
        <input type="text" data-env-input="${e.id}" placeholder="https://api.example.com" value="${escapeHtml(val)}">
        <button type="button" class="esc-copy" data-env-copy="${e.id}" title="Copy base URL">⧉</button>
      </div>
    </div>`;
  }).join('');
...
function populateFlowDirectionSelect(selectedId){
  const sel = document.getElementById('pgFlowDirection');
  const dirs = allFlowDirections();
  sel.innerHTML = dirs.map(d=>`<option value="${d.id}">${escapeHtml(d.label)}</option>`).join('')
    + `<option value="__add_custom__">+ Add custom direction…</option>`;
  sel.value = dirs.some(d=>d.id===selectedId) ? selectedId : dirs[0].id;
}

/* ---------- Auth tab: request/response parameters (table view only) ---------- */
let authReqParams = [];
let authRespParams = [];
// Tracks the last auto-generated JSON string per box, so we only overwrite the
// editable textarea when the user hasn't typed their own custom example — the
// same "auto-generated but you can hand-edit it" behavior as the endpoint's
// Request JSON box (regenerateRequestJson), just tracked explicitly here since
// this box doesn't regenerate on every keystroke.
...
function renderAuthParamRows(kind){
  const rows = kind === 'req' ? authReqParams : authRespParams;
  const tbody = document.getElementById(kind === 'req' ? 'authReqRows' : 'authRespRows');
  if(!tbody) return;
  if(!rows.length){
  ...
  tbody.querySelectorAll('[data-del]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      const id = btn.getAttribute('data-del');
      if(kind === 'req') authReqParams = authReqParams.filter(r=>r.id!==id);
      else authRespParams = authRespParams.filter(r=>r.id!==id);
      renderAuthParamRows(kind);
    });
  });
  updateAuthJsonPreview(kind);
}
...
function updateAuthJsonPreview(kind){
  const ta = document.getElementById(kind === 'req' ? 'authReqJsonPreview' : 'authRespJsonPreview');
  if(!ta) return;
  const rows = kind === 'req' ? authReqParams : authRespParams;
  const generated = JSON.stringify(buildJsonFromRows(rows), null, 2);
  if(ta.value === '' || ta.value === authJsonAutoValue[kind]){
    ta.value = generated;
  }
...
document.getElementById('addAuthReqRow').addEventListener('click', ()=>{ authReqParams.push(newAuthParamRow()); renderAuthParamRows('req'); });
document.getElementById('addAuthRespRow').addEventListener('click', ()=>{ authRespParams.push(newAuthParamRow()); renderAuthParamRows('resp'); });
...
function openProjectModal(projectId, initialTab){
  ...
  document.getElementById('pgName').value = proj.name;
  document.getElementById('pgDesc').value = proj.description || '';
  document.getElementById('pgDesc').classList.remove('hidden-src');
  document.getElementById('pgDescPreview').classList.remove('show');
  document.getElementById('pgDescPreviewToggle').textContent = 'Preview';
  document.getElementById('pgLifecycle').innerHTML = LIFECYCLE_STAGES.map(s=>`<option value="${s}">${s}</option>`).join('');
  document.getElementById('pgLifecycle').value = proj.lifecycle || 'PRODUCTION';
  populateFlowDirectionSelect(proj.requestFlowDirection || '1-way');
  document.getElementById('flowCustomBuilder').style.display = 'none';
  document.getElementById('flowCustomLabel').value = '';
  document.getElementById('pgOwner').value = proj.owner || '';
  document.getElementById('pgTeam').value = proj.team || '';
  document.getElementById('pgTerms').value = proj.termsOfService || '';
  document.getElementById('pgContactName').value = (proj.contact && proj.contact.name) || '';
  document.getElementById('pgContactEmail').value = (proj.contact && proj.contact.email) || '';
  document.getElementById('pgLicenseName').value = (proj.license && proj.license.name) || '';
  document.getElementById('pgLicenseUrl').value = (proj.license && proj.license.url) || '';
  document.getElementById('paType').value = (proj.auth && proj.auth.type) || '';
  document.getElementById('paMethod').value = (proj.auth && proj.auth.method) || 'POST';
  document.getElementById('paPath').value = (proj.auth && proj.auth.path) || '';
  document.getElementById('paHeader').value = (proj.auth && proj.auth.headerName) || '';
  document.getElementById('paDesc').value = (proj.auth && proj.auth.description) || '';
  document.getElementById('paIncludeDocs').checked = !!(proj.auth && proj.auth.includeInDocs);
  document.getElementById('paIncludeSwagger').checked = !!(proj.auth && proj.auth.includeInSwagger);
  authReqParams = cloneAuthParams(proj.auth && proj.auth.requestParams);
  authRespParams = cloneAuthParams(proj.auth && proj.auth.responseParams);
  renderAuthParamRows('req');
  renderAuthParamRows('resp');
  // Restore any hand-edited JSON example saved with the project (falls back to
  // what renderAuthParamRows already auto-generated above if none was saved).
  ...
  renderEnvSettingsCards(proj);
  renderProjectDocsList(proj);

  const tab = initialTab || 'general';
  ...
  listEl.innerHTML = docs.map(d=>{
    ...
    <div class="doc-row">
      ...
      <div class="doc-row-actions">
        <a class="icon-btn" href="${d.dataUrl}" download="${escapeHtml(d.name)}" title="Download">${ICON_DOWNLOAD}</a>
...
function saveProjectSettings(){
  const proj = state.projects[editingProjectId];
  if(!proj) return;
  const AUDIT_FIELD_LABELS = { name:'Name', description:'Description', lifecycle:'Lifecycle', owner:'Owner', team:'Team',
    termsOfService:'Terms of service', contact:'Contact', license:'License', auth:'Auth', environments:'Environment URLs',
    requestFlowDirection:'Request flow direction' };
  const snapshot = () => ({
    name: proj.name, description: proj.description, lifecycle: proj.lifecycle,
    owner: proj.owner, team: proj.team, termsOfService: proj.termsOfService,
    contact: JSON.stringify(proj.contact), license: JSON.stringify(proj.license),
    auth: JSON.stringify(proj.auth), environments: JSON.stringify(proj.environments),
    requestFlowDirection: proj.requestFlowDirection,
  });
  const before = snapshot();
  proj.name = document.getElementById('pgName').value.trim() || proj.name;
  proj.description = document.getElementById('pgDesc').value.trim();
  proj.lifecycle = LIFECYCLE_STAGES.includes(document.getElementById('pgLifecycle').value)
    ? document.getElementById('pgLifecycle').value : proj.lifecycle;
  const flowVal = document.getElementById('pgFlowDirection').value;
  proj.requestFlowDirection = (flowVal && flowVal !== '__add_custom__') ? flowVal : (proj.requestFlowDirection || '1-way');
  proj.owner = document.getElementById('pgOwner').value.trim();
  proj.team = document.getElementById('pgTeam').value.trim();
  proj.termsOfService = document.getElementById('pgTerms').value.trim();
  proj.contact = {
    name: document.getElementById('pgContactName').value.trim(),
    email: document.getElementById('pgContactEmail').value.trim(),
  };
  proj.license = {
    name: document.getElementById('pgLicenseName').value.trim(),
    url: document.getElementById('pgLicenseUrl').value.trim(),
  };
  const cleanAuthParams = (list)=> list.filter(r=>r.name.trim()).map(r=>(
    { name:r.name.trim(), type:r.type, required:!!r.required, example:r.example.trim(), description:r.description.trim() }
  ));
  proj.auth = {
    type: document.getElementById('paType').value.trim(),
    method: document.getElementById('paMethod').value,
    path: document.getElementById('paPath').value.trim(),
    headerName: document.getElementById('paHeader').value.trim(),
    description: document.getElementById('paDesc').value.trim(),
    requestParams: cleanAuthParams(authReqParams),
    responseParams: cleanAuthParams(authRespParams),
    // Whatever's currently sitting in the editable JSON boxes — auto-generated
    // or hand-edited, same as how the endpoint's Request JSON body is saved.
    requestExample: (document.getElementById('authReqJsonPreview').value || '').trim(),
    responseExample: (document.getElementById('authRespJsonPreview').value || '').trim(),
    includeInDocs: document.getElementById('paIncludeDocs').checked,
    includeInSwagger: document.getElementById('paIncludeSwagger').checked,
  };
  document.querySelectorAll('[data-env-input]').forEach(inp=>{
    proj.environments[inp.getAttribute('data-env-input')] = inp.value.trim();
  });
  const after = snapshot();
  const changedFields = Object.keys(before).filter(k=>before[k]!==after[k]);
  if(changedFields.length){
    logAudit('updated', 'project', proj.name, `Updated ${changedFields.map(f=>AUDIT_FIELD_LABELS[f]||f).join(', ')}`, proj.name);
  }
  proj.updatedAt = new Date().toISOString();
  saveState();
  closeProjectModal();
  ...
  return {
    ...
    requestBody: document.getElementById('mBody').value.trim()
      ? {
          ...
          examples: builderReqExamples.filter(ex=>ex.name.trim() && ex.value.trim()).map(ex=>({ name:ex.name.trim(), value:ex.value.trim() })),
        } : null,
...
function openTryItModal(proj, ep){
  ...
  const baseUrl = proj.environments[state.env] || '';
  document.getElementById('tryItUrlPreview').textContent = `${envMeta(state.env).label} → ${baseUrl}${ep.path}`;

  const pathParams = (ep.parameters||[]).filter(p=>p.in==='path');
  const queryParams = (ep.parameters||[]).filter(p=>!p.in || p.in==='query');
  const allParams = [...pathParams, ...queryParams];

  const paramsEl = document.getElementById('tryItParams');
  const noParamsEl = document.getElementById('tryItNoParams');
  if(!allParams.length){
    paramsEl.innerHTML = '';
    noParamsEl.style.display = '';
  } else {
    noParamsEl.style.display = 'none';
    paramsEl.innerHTML = allParams.map(p=>`
      <div>
        <label style="font-size:11px; color:var(--text-faint); display:flex; gap:6px; margin-bottom:3px;">
          ${escapeHtml(p.name)}${p.required ? ' <span style="color:var(--danger);">*</span>' : ''}
          <span class="mono" style="color:var(--text-faint);">${p.in==='path' ? 'path' : 'query'} · ${escapeHtml(p.type||'string')}</span>
        </label>
        <input type="text" data-try-param="${escapeHtml(p.name)}" data-try-in="${p.in==='path'?'path':'query'}"
               value="${escapeHtml(String(exampleValueFor(p.type, p.example)))}" placeholder="${escapeHtml(p.description||'')}">
      </div>`).join('');
  }

  document.getElementById('tryItResult').style.display = 'none';
  document.getElementById('tryItSend').disabled = false;
  document.getElementById('tryItSend').textContent = 'Send';
...
function sendTryIt(){
  ...
  const inputs = document.querySelectorAll('[data-try-param]');
  for(const input of inputs){
    const name = input.getAttribute('data-try-param');
    const param = (ep.parameters||[]).find(p=>p.name===name);
    if(param && param.required && !input.value.trim()){
  ...
  setTimeout(()=>{
    const r = (ep.responses||[]).find(r=>String(r.code)[0]==='2') || (ep.responses||[])[0];
    ...
    if(!r){
      codeEl.textContent = '—';
      codeEl.className = 'resp-code';
      bodyEl.textContent = 'No response documented for this endpoint yet — add one from Edit.';
    } else {
      codeEl.textContent = String(r.code);
      codeEl.className = 'resp-code ' + respClass(r.code);
      const parsed = r.example ? tryParseJson(r.example) : { ok:false };
      bodyEl.textContent = parsed.ok ? JSON.stringify(parsed.value, null, 2) : (r.example || 'No example body documented for this response.');
    }
    latencyEl.textContent = `Simulated from documented example · ${Math.round(180 + Math.random()*140)} ms`;
...
<div class="al-wrap">
  <div class="al-header">
    <div class="al-title">
      ...
      <div>
        <h1>Audit Log</h1>
        <div class="sub">API Studio · every create, edit, and delete — who did it, and when · last activity ${escapeHtml(lastWhen)}</div>
      </div>
    </div>
  </div>
